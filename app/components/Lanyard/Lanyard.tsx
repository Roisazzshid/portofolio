...
export default function Lanyard({
  ...
  useEffect(() => {
    ...
    const resetPositions = () => {
      refs.current.fixed?.setTranslation({ x: 0, y: 4, z: 0 });
      refs.current.j1?.setTranslation({ x: 0.5, y: 4, z: 0 });
      refs.current.j2?.setTranslation({ x: 1, y: 4, z: 0 });
...
function Band({ maxSpeed = 50, minSpeed = 0, refs }: BandProps) {
  ...
  // simpan ref ke parent buat reset
  useEffect(() => {
    refs.current.fixed = fixed.current;
    refs.current.j1 = j1.current;
    refs.current.j2 = j2.current;
    refs.current.j3 = j3.current;
    refs.current.card = card.current;
  }, []);
